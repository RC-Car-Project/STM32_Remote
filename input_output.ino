...
void IO_update() {
    if(lastIO + 20 < millis()) {
      lastIO = millis();
        ...
        nrf24l01_s(2,convertPot(2, pot[2]));
          
        pot[1] = analogRead(pin_POT_1);
        potDirSpe = convertPot(1, pot[1]);
        nrf24l01_s(1,potDirSpe);
        ...
        if(digitalRead(pin_BTN_3) == LOW) {
          ...
          updateIntPot(255-pot[0]);
        }else if(digitalRead(pin_BTN_1) == LOW && !useBle) {
          useBle = true;
          buzzer(30);

        }else if(digitalRead(pin_BTN_0) == LOW && useBle) {
          useBle = false;
          buzzer(30);
          delay(30);
          buzzer(30);
      ...
      /*#if is_test_mode == true
        serial_FTDI.print("IO: ");
        serial_FTDI.print("B0 "+String(digitalRead(pin_BTN_0))+", ");
        serial_FTDI.print("B1 "+String(digitalRead(pin_BTN_1))+", ");
        serial_FTDI.print("B2 "+String(digitalRead(pin_BTN_2))+", ");
        serial_FTDI.print("B3 "+String(digitalRead(pin_BTN_3))+", ");
        serial_FTDI.print("P0 "+String(analogRead(pin_POT_0))+", ");
        serial_FTDI.print("P1 "+String(analogRead(pin_POT_1))+", ");
        serial_FTDI.print("P2 "+String(analogRead(pin_POT_2)));
        serial_FTDI.println(" ");
      #endif */
    }
}
